...
begin
	...
	while @i < 30000
	begin
			insert into Tc values ( @i % 10000 + 1, @i % 3000 + 1) 
			set @i = @i + 1
	end
end
go
...
select idB from Tb -- dc nu face scan aici? i-am dat o conditie care nu e restrictiva... pt ca POATE efectua cautarea necesara (solicitata de mine, user-ul) dupa ce are in index.. ca nu e 
...
-- c)
--select * from Tb
-- select * from Tb where b2 % 10 = 0 -- a facut scan aici la un moment dat pentru ca la momentul acela nu avea alt index existent
...
/*
select Tc.idC, Tc.idA, Ta.a2 from Tc
...
select Tc.idC, Tc.idA, Tb.b2 from Tc
...
-- Timpul de cautare pe arbore s-a redus de la 0.09 la 0.03, deci de aproape 3 ori mai bine.



select Tc.idB from Tc
inner join Tb on Tc.idB = Tb.idb
where Tb.idB = 1000                         -- < 9000
